...
class Certifications extends React.Component{
    ...
    render(){
        var certifications = Object.keys(this.props.data.certifications).map(x =>  <li className="list-group-item">
        <a href={this.props.data.certifications[x].link} target="_blank" rel="noopener noreferrer"> 
          {x}</a>
          <p class="by">{this.props.data.certifications[x].by}</p>
          <p class="date">{this.props.data.certifications[x].date}</p>
        </li>);
        return (
